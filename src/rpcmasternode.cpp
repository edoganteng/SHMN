...
// Copyright (c) 2018 The shmn developers
...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <shmnaddress> <amount>\n"
            "shmnaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <shmnaddress> <amount>\n"
            "shmnaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid shmn address");
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in shmn.conf\n"
...
Value masternodelist(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1)) {
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode shmn address\n"
